...
| **inflate** | 🟢 | deflate **5,5×–7,1× vs node:zlib** (19×–35× vs pako); inflate **1,1×–1,7× vs node:zlib** (5,6×–6,5× vs pako) | Re-Messung 2026-04-21: `decompress_bulk` + uninit-output-buffer-Fix hat inflate-Regression geheilt (0,46× → 1,12×+ auf allen Größen). Kein Szenario unter 1× gegen node:zlib, durchweg dominant vs pako. Upgrade Yellow → Green. (Commit `32d7dfa`; Review `docs/perf-review/inflate.md`) |
...
**Net (Stand nach den Re-Reviews 2026-04-21):** 5 Green → **13 Green** + 1 faktisch-Green (nanoid). 7 Yellow → **2 Yellow** (argon2, bcrypt — beide algorithmisch/Backend-limitiert, 2× Gate strukturell nicht erreichbar). 3 Red → 3 Deprecated (3-Monats-Window). Portfolio-Total: 16 shipped.

**Update 2026-04-19 (Perf-Sprint)**: encoding von Yellow → Green nach shift_jis Re-Messung (1,17× statt 0,65×). Inflate blieb Yellow mit laufendem Backend-Spike (`docs/perf-review/inflate-backend-spike.md`); zusätzlich `decompress_bulk` ohne Zero-Init-Output-Buffer (erwartet +5–15 % auf 10 MB Inflate). Neue additive APIs im Portfolio: `renderFast`/`renderBytesFast` — jeweils Option-Unmarshalling-frei für FFI-floor-dominierte Cases. file-type async bounded copy auf 4 KB Prefix (vorher: voller Buffer).

**Update 2026-04-21 (Re-Review-Sweep)**: inflate von Yellow → Green nach Re-Messung gegen aktuelle `docs/data.json`-Shards. Die Phase-C-Fixes (Commit `32d7dfa`: `decompress_bulk` + uninit-output-buffer) haben die inflate-Regression geheilt — 1,12×–1,67× gegen node:zlib, 5,6×–6,5× gegen pako, kein Szenario unter 1×. Portfolio hat jetzt **13 klare Green + 1 faktisch-Green (nanoid) + 2 Yellow (argon2, bcrypt)**. Zusätzlich: alle 16 shipped Crates haben jetzt eigene `docs/perf-review/<name>.md` mit Phase-C-Lever-Checklist.

Die 13-Green-Packages sind alle netto-schneller-als-JS auf jedem gemessenen Szenario. Das ist die Qualitätsgarantie für das Portfolio: keine Footguns mehr, keine "works well for X but slow for Y"-Überraschungen in den Green-Packages.
