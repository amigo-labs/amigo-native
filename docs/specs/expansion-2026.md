...
Branch: `claude/expand-amigo-native-Q64Pf` — PR
[#98](https://github.com/amigo-labs/amigo-native/pull/98) (draft).

### Commits on the branch, newest first

| SHA       | Subject                                                        |
| :-------- | :------------------------------------------------------------- |
| `69116f5` | fix(crates): regenerate index.js with current versions across 26 crates |
| `53a582c` | feat(slugify): dual-target pilot — core/napi split + WASM binding |
| `cad60b2` | docs(specs): incorporate decisions D1/D2/D3 into expansion-2026 |
| `d9875ee` | docs(specs): add expansion-2026 dual-target plan (draft)       |

### Done

- Spec written and revised against the actual 36-crate workspace.
- Architecture decisions D1–D6 captured (one-package conditional
  exports, warn-then-hard bundle budget, deferred search index
  portability, nested `wasm/` workspace member with `cdylib + rlib`,
  workspace-root `vite-plugin-wasm`, post-build `wasm-opt`).
- **slugify dual-target pilot** end-to-end: `_slugify-core` extracted,
  napi slimmed to wrappers, `crates/slugify/wasm/` shipping
  wasm-bindgen output, conditional exports in `package.json`. All
  acceptance gates green: 82/82 vitest, 3/3 wasm-bindgen-test (Node),
  `cargo test --workspace`, `cargo fmt`, `cargo clippy -D warnings`
  on native + wasm32 targets. Bench: WASM 2–3× faster than JS
  `slugify`, payload 246 KB gzipped unoptimized (expected ~80–120 KB
  after `wasm-opt -Oz`).
- Workspace plumbing in place: extended `members` glob,
  `crates/*/wasm/pkg/` gitignored, `vite-plugin-wasm` at workspace
  root, `vitest.config.ts` updated.
- Workspace-wide `index.js` version drift fixed across 26 crates
  (closes the `index.js` side of `docs/code-review-2026-04-25.md`
  finding #3).

### Open (non-blocking)

- **Q1 — xxhash SIMD.** Decide during the `xxhash` WASM PR with real
  benchmark data. Likely outcome: ship `+simd128` by default, fall
  back to scalar if `wasm-opt` can't preserve the SIMD intrinsics.
- **Q2 — commonmark XSS guidance wording.** Decide during the
  `commonmark` WASM PR. Crate README is the primary location; the
  dashboard surfaces the same text.

### Pick up here in the next session

In rough priority order:

1. **Pick the next shortlist crate** and apply the slugify pilot
   pattern. Two reasonable starting points:
   - `xxhash` — pure compute, smallest API surface after slugify,
     and the run will produce the data needed to close Q1 (SIMD).
     Lowest risk, fastest feedback.
   - `sanitize-html` — highest user value (browser HTML sanitization
     is a frequent ask), but highest bundle-size risk
     (`ammonia` + `html5ever` typically 200–400 KB gzipped); if it
     blows the budget the spec's "warn-only first" decision (D2)
     gets exercised early.

   Recommendation: do `xxhash` first to validate the pattern on a
   second crate cheaply, then `sanitize-html` as the first real
   stress test of the budget.

2. **Wire up CI for WASM** (Phase 4 of the spec). Minimum viable:
   - Job `wasm-test` running `wasm-pack test --node` per crate that
     has a `wasm/` sub-crate. Matrix-fan out as more crates land.
   - Job `wasm-size` building each WASM artifact with `wasm-opt -Oz`
     (system `binaryen`) and reporting gzipped bytes per crate.
     Warn-only initially (per D2). Capture baselines in
     `docs/data.json` or similar.
   - Add `wasm32-unknown-unknown` toolchain + `wasm-pack` install
     to the existing CI job setup.

3. **Follow-up PR: parent-vs-index.js CI guard.** One-liner in
   `scripts/sync-registry.mjs` that fails if any crate's `index.js`
   embeds a different version than its `package.json`. Prevents
   recurrence of the drift that was fixed in `69116f5`. Best done as
   its own PR after this one merges.

4. **Decide on README updates.** When the slugify pilot ships, the
   `crates/slugify/README.md` should grow an "Install for the
   browser" subsection that explains: no separate `-wasm` package,
   same `npm install @amigo-labs/slugify`, bundler picks the right
   target via `exports`. Pattern for the rest of the shortlist.

5. **Decide on dashboard surface.** `docs/packages.json` needs a
   `targets: ["node", "browser"]` field per dual-target crate, and
   the dashboard (`docs/app.js` + `docs/index.html`) needs a small
   UI affordance to display it. Defer until at least two crates
   ship WASM so the affordance has something to show.

### Things deliberately not done

- No edits to `CLAUDE.md`. The existing file is minimal on purpose;
  the conventions live in this spec.
- No edits to `README.md` tagline ("Rust-powered npm packages,
  Node + Browser"). Defer until the first WASM crate ships and the
  claim is honest.
- No new `BACKLOG.md` entries. The two open questions are tracked in
  this doc; CI/registry follow-ups are described above.
