# Parity tests — deep-equal

This directory verifies that `@amigo-labs/deep-equal` behaves compatibly with the
...
```bash
# from repo root:
pnpm test:parity --filter @amigo-labs/deep-equal

# or per-package:
cd crates/deep-equal && pnpm test:parity
```
