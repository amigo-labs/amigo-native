# Parity tests — levenshtein

This directory verifies that `@amigo-labs/levenshtein` behaves compatibly with the
...
```bash
# from repo root:
pnpm test:parity --filter @amigo-labs/levenshtein

# or per-package:
cd crates/levenshtein && pnpm test:parity
```
