...
mod tests {
    ...
    #[test]
    fn split_text_respects_overlap() {
        let input = "abcdefghij ".repeat(200);
        let chunks = split_text(
            input,
            Some(SplitterOptions {
                chunk_size: Some(100),
    ...
    fn count_chars_works() {
        assert_eq!(count_chars("hello".to_string()), 5);
    }

    #[test]
    fn count_tokens_returns_non_zero() {
        let n = count_tokens("hello world".to_string(), None).unwrap();
        assert!(n >= 1);
    }
    ...
    fn invalid_metric_errors() {
        let result = split_text(
            "x".to_string(),
            Some(SplitterOptions {
                length_metric: Some("bogus".to_string()),
                ..Default::default()
            }),
        );
    ...
    #[test]
    fn batch_works() {
        let docs = vec!["hello world".to_string(), "a b c d e f g h".to_string()];
        let out = split_text_batch(
            docs,
            Some(SplitterOptions {
                chunk_size: Some(100),
                chunk_overlap: Some(0),
    ...
    fn token_metric_splits() {
        ...
        let chunks = split_text(
            ...
            Some(SplitterOptions {
                chunk_size: Some(20),
                chunk_overlap: Some(0),
                length_metric: Some("tiktoken:cl100k_base".to_string()),
            }),
        )
        .unwrap();
        assert!(chunks.len() > 1);
