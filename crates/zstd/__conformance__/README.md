# Conformance — `@amigo-labs/zstd`

This directory verifies that `@amigo-labs/zstd` behaves compatibly with the
...
```bash
# from repo root:
pnpm --filter @amigo-labs/zstd test:conformance

# or per-package:
cd crates/zstd && pnpm test:conformance

# everything (unit + conformance):
pnpm --filter @amigo-labs/zstd test:all
```
