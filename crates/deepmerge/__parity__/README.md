# Parity tests — deepmerge

This directory verifies that `@amigo-labs/deepmerge` behaves compatibly with the
...
```bash
# from repo root:
pnpm test:parity --filter @amigo-labs/deepmerge

# or per-package:
cd crates/deepmerge && pnpm test:parity
```
