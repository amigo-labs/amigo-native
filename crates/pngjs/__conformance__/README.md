# Conformance — `@amigo-labs/pngjs`

This directory verifies that `@amigo-labs/pngjs` behaves compatibly with the
...
```bash
# from repo root:
pnpm --filter @amigo-labs/pngjs test:conformance

# or per-package:
cd crates/pngjs && pnpm test:conformance

# everything (unit + conformance):
pnpm --filter @amigo-labs/pngjs test:all
```
