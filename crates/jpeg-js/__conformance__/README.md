# Conformance — `@amigo-labs/jpeg-js`

This directory verifies that `@amigo-labs/jpeg-js` behaves compatibly with the
...
```bash
# from repo root:
pnpm --filter @amigo-labs/jpeg-js test:conformance

# or per-package:
cd crates/jpeg-js && pnpm test:conformance

# everything (unit + conformance):
pnpm --filter @amigo-labs/jpeg-js test:all
```
