# Conformance — `@amigo-labs/linkify-it`

This directory verifies that `@amigo-labs/linkify-it` behaves compatibly with the
...
```bash
# from repo root:
pnpm --filter @amigo-labs/linkify-it test:conformance

# or per-package:
cd crates/linkify-it && pnpm test:conformance

# everything (unit + conformance):
pnpm --filter @amigo-labs/linkify-it test:all
```
