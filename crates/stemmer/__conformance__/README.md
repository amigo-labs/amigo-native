# Conformance — `@amigo-labs/stemmer`

This directory verifies that `@amigo-labs/stemmer` behaves compatibly with the
...
```bash
# from repo root:
pnpm --filter @amigo-labs/stemmer test:conformance

# or per-package:
cd crates/stemmer && pnpm test:conformance

# everything (unit + conformance):
pnpm --filter @amigo-labs/stemmer test:all
```
