...
Both one-shot functions and the stateful hashers ship to the browser. 64-bit hashes come back as JS `BigInt` directly. WASM is faster than `xxhash-wasm` / `xxhashjs` on typical inputs; SIMD (`+simd128`) is deferred per the expansion-2026 spec open question Q1. Live speedup numbers live on the [dashboard](https://amigo-native.amigo-labs.workers.dev/).
