# Parity tests — file-type

This directory verifies that `@amigo-labs/file-type` behaves compatibly with the
...
```bash
# from repo root:
pnpm test:parity --filter @amigo-labs/file-type

# or per-package:
cd crates/file-type && pnpm test:parity
```
