# Conformance — `@amigo-labs/diff`

This directory verifies that `@amigo-labs/diff` behaves compatibly with the
...
```bash
# from repo root:
pnpm --filter @amigo-labs/diff test:conformance

# or per-package:
cd crates/diff && pnpm test:conformance

# everything (unit + conformance):
pnpm --filter @amigo-labs/diff test:all
```
