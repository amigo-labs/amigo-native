# Conformance — `@amigo-labs/jimp`

This directory verifies that `@amigo-labs/jimp` behaves compatibly with the
...
```bash
# from repo root:
pnpm --filter @amigo-labs/jimp test:conformance

# or per-package:
cd crates/jimp && pnpm test:conformance

# everything (unit + conformance):
pnpm --filter @amigo-labs/jimp test:all
```
