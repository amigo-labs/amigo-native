# Conformance — `@amigo-labs/fuse`

This directory verifies that `@amigo-labs/fuse` behaves compatibly with the
...
```bash
# from repo root:
pnpm --filter @amigo-labs/fuse test:conformance

# or per-package:
cd crates/fuse && pnpm test:conformance

# everything (unit + conformance):
pnpm --filter @amigo-labs/fuse test:all
```
