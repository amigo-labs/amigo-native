# Conformance — `@amigo-labs/tldts`

This directory verifies that `@amigo-labs/tldts` behaves compatibly with the
...
```bash
# from repo root:
pnpm --filter @amigo-labs/tldts test:conformance

# or per-package:
cd crates/tldts && pnpm test:conformance

# everything (unit + conformance):
pnpm --filter @amigo-labs/tldts test:all
```
