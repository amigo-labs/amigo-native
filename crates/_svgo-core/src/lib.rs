...
pub fn optimize(svg: &str, config: &SvgoConfig) -> SvgoResult {
    let resolved: Resolved = config.into();
    let input_bytes = svg.len() as u32;

    let mut data = optimize_pass(svg, &resolved);
    if config.multipass.unwrap_or(false) {
        for _ in 0..4 {
...
pub fn optimize_many(svgs: &[String], config: &SvgoConfig) -> Vec<SvgoResult> {
    svgs.iter().map(|s| optimize(s, config)).collect()
}
...
mod tests {
    ...
    fn opt(s: &str) -> SvgoResult {
        optimize(s, &SvgoConfig::default())
    }

    #[test]
    fn removes_comments() {
        let res = opt("<svg><!-- comment --><rect/></svg>");
        assert!(!res.data.contains("comment"));
    }

    #[test]
    fn removes_metadata_title_desc() {
        let res = opt("<svg><title>X</title><desc>d</desc><metadata>m</metadata><rect/></svg>");
    ...
    fn strips_empty_attrs() {
        let res = opt("<svg><rect id=\"\" width=\"10\"/></svg>");
    ...
    #[test]
    fn cleanup_numbers() {
        let res = opt("<svg><rect width=\"10.123456\" height=\"5.000000\"/></svg>");
    ...
    #[test]
    fn removes_hidden() {
        let res = opt("<svg><rect display=\"none\" width=\"10\"/><circle r=\"5\"/></svg>");
    ...
    fn converts_named_colors() {
        let res = opt("<svg><rect fill=\"black\" stroke=\"white\"/></svg>");
    ...
    fn converts_rgb_to_hex() {
        let res = opt("<svg><rect fill=\"rgb(255, 0, 0)\"/></svg>");
        assert!(res.data.contains("#f00"));
    }

    #[test]
    fn removes_useless_defs() {
        let res = opt("<svg><defs></defs><rect/></svg>");
        assert!(!res.data.contains("defs"));
    }

    #[test]
    fn collapses_groups() {
        let res = opt("<svg><g><rect width=\"10\"/></g></svg>");
    ...
    fn removes_editor_ns() {
        let res =
            opt("<svg xmlns:sodipodi=\"x\"><rect sodipodi:nodetypes=\"cc\" width=\"10\"/></svg>");
    ...
    fn reports_savings() {
        let res =
            opt("<svg>    <!-- comment -->  <rect width=\"1.000000\" fill=\"black\"/>  </svg>");
    ...
    fn batch_optimize() {
        ...
        let results = optimize_many(&svgs, &SvgoConfig::default());
