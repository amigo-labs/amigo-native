# Conformance — `@amigo-labs/language-detect`

This directory verifies that `@amigo-labs/language-detect` behaves compatibly with the
...
```bash
# from repo root:
pnpm --filter @amigo-labs/language-detect test:conformance

# or per-package:
cd crates/language-detect && pnpm test:conformance

# everything (unit + conformance):
pnpm --filter @amigo-labs/language-detect test:all
```
