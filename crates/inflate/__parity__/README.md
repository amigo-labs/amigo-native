# Parity tests — inflate

This directory verifies that `@amigo-labs/inflate` behaves compatibly with the
...
```bash
# from repo root:
pnpm test:parity --filter @amigo-labs/inflate

# or per-package:
cd crates/inflate && pnpm test:parity
```
