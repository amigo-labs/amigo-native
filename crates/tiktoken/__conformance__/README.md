# Conformance — `@amigo-labs/tiktoken`

This directory verifies that `@amigo-labs/tiktoken` behaves compatibly with the
...
```bash
# from repo root:
pnpm --filter @amigo-labs/tiktoken test:conformance

# or per-package:
cd crates/tiktoken && pnpm test:conformance

# everything (unit + conformance):
pnpm --filter @amigo-labs/tiktoken test:all
```
